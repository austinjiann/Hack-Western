  ...
  useEffect(() => {
    intervalRef.current = setInterval(async () => {
      ...
      const arrows = editor
        ...
        .filter(
          (s) =>
            s.type === "arrow" && s.meta?.jobId && s.meta?.status === "pending"
        );
    ...
    if (sceneResp.ok) {
      try {
        ...
        addClip({
          ...
          lastFrameUrl: String(arrow.meta.lastFrameUrl ?? ""),
          annotations: extracted,
          prompt: String(arrow.meta.prompt),
          modelParams: arrow.meta.modelParams ?? {},
        });
      } catch (parseError) {
